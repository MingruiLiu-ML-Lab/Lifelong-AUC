...
from .utils import Reservoir, RingBuffer, ClassBalanced, RM
...
class Net(nn.Module):
    def __init__(self,
        ...
        imratio_list = [1 / self.nc_per_task] * n_outputs
        imratio_list_w = [1 - args.imratio, args.imratio] * (n_outputs // 2)
        self.loss_fn_w = AUCM_MultiLabel(imratio=imratio_list_w, num_classes=n_outputs)
        self.loss_fn_v = AUCM_MultiLabel(imratio=imratio_list, num_classes=n_outputs)
        ...
        self.memory = RM(n_tasks * self.n_memories, (3, n_inputs, n_inputs), n_outputs * 2)
    ...
    def observe(self, x, t, y):
        ...
        mask = self.label_mask[m_y].bool()
        output = output * mask
        score = torch.sigmoid(output)
        loss_v += self.loss_fn_v(score, m_y)
